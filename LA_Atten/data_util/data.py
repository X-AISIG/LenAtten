...
class Vocab(object):
  """
  Generate word Dictionary with max_size.
  """
  def __init__(self, vocab_file, max_size):
    ...
    with open(vocab_file, 'r') as vocab_f:
      ...
      for line in vocab_f:
        ...
        # If word is not a normal word.
          raise Exception('<s>, </s>, [UNK], [PAD], [START] and [STOP] shouldn\'t be in the vocab file, but %s is' % w)
        if w in self._word_to_id:
        # The word is already in the word dictionary.
  ...
  def word2id(self, word):
    """
    Return the id (integer) of given word.
    If the word is not in the word dictionary, return the id of UNK.
    """
  ...
  def id2word(self, word_id):
    """
    Return the word (string) of corresponding to a word id.
    If the word id is not in the word_id dictionary, an exception will be raised.
    """
  ...
  def size(self):
    """
    Return the size of word dictionary.
    """
    return self._count

# Following code is not used in our implementation.
# ------------------------------------------------
...
def example_generator(data_path, single_pass):
  ...
  Args:
    data_path:
      Path to tf.Example data files. Can include wildcards, 
      e.g. if you have several training data chunk files train_001.bin, train_002.bin, etc, 
...
def article2ids(article_words, vocab):
  ...
  Returns:
    ids:
      ...
      OOVs are represented by their temporary article OOV number.
...
def abstract2ids(abstract_words, vocab, article_oovs):
  """
  Map the abstract words to their ids.
  In-article OOVs are mapped to their temporary OOV numbers.
  ...
  Returns:
    ...
    In-article OOV words are mapped to their temporary OOV numbers. Out-of-article OOV words are mapped to the UNK token id."""
  ids = []
  unk_id = vocab.word2id(UNKNOWN_TOKEN)
...
def outputids2words(id_list, vocab, article_oovs):
  """Maps output ids to words, 
    including mapping in-article OOVs from their temporary ids
  ...
  Args:
    ...
    article_oovs: list of OOV words (strings) in the order corresponding
      to their temporary article OOV ids (that have been assigned in pointer-generator mode), or None (in baseline mode)
...
# Following code is not used in our implementation.
# For display generated sentense, could be ignored.
# ------------------------------------------------
