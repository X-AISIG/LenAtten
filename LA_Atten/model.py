...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.sinu_embedding_look_up_table = None
    ...
    def forward(self, article_list, article_lens, pack_article_lens):
        """
        Args:
            article_list: A list contains batch-size of articles (each of the word of each article is a word-embedded vector).
            article_lens: A 2-D list contains the character length of batch size articles.
        
        Returns:
            enc_out: A list contains all of the encoder hidden states of batch size articles.
            h_reduced: reduced final hidden states of Bi-LSTM output.
            c_reduced: reduced final cell units of Bi-LSTM output.
        """

        packed = pack_padded_sequence(article_list, pack_article_lens, batch_first=True)
        ...
        # Reduce the dimension into n_hid, and used to initialize the decoder LSTM.
...
class encoder_attention(nn.Module):
    ...
    def __init__(self):
        ...
        # hidden_dim * 2 is because we use Bi-LSTM in our model, and concatenate
        # Bi-LSTM's result to form an encoder hidden states.
...
class len_attention(nn.Module):
    def __init__(self):
        super(len_attention, self).__init__()
        self.W_l = nn.Linear(1, config.hidden_dim, bias=False)
        self.W_s = nn.Linear(config.hidden_dim, config.hidden_dim)
        self.L = nn.Linear(config.hidden_dim, config.max_word_len, bias=False)

    def forward(self, s_t, remain_length, character_attn_table, size):

        character_attn_table = character_attn_table.repeat(size, 1, 1)

        ct = get_cuda(character_attn_table)
        
        if not isinstance(remain_length, T.Tensor):
            tensor_remain_len = T.from_numpy(remain_length)
        else:
            tensor_remain_len = remain_length

        tensor_remain_len = tensor_remain_len.type(T.float)
        tensor_remain_len = tensor_remain_len.view(-1, 1)

        tensor_remain_len /= config.div_factor
        tensor_remain_len = get_cuda(tensor_remain_len)

        et = self.W_l(tensor_remain_len)           
        dec_fea = self.W_s(s_t)
        et = (et + dec_fea).unsqueeze(1)

        et = T.tanh(et)                         
        et = self.L(et).squeeze(1)              
        et = et.type(T.float)

        at = F.softmax(et, dim=1).unsqueeze(1)  
        ct_l = T.bmm(at, ct).squeeze(1)     
        return ct_l, at

class Decoder(nn.Module):
    def __init__(self):
        super(Decoder, self).__init__()
        self.dec_lstm_len_embedding_look_up_table = None

        # length attention table:
        self.len_attn_table = sinu_embedding.create_len_attn_table()

        self.enc_attention = encoder_attention()
        self.dec_attention = decoder_attention()
        self.len_attention = len_attention()
        ...
        self.V = nn.Linear(config.hidden_dim * 5, config.hidden_dim)
        # Change to * 5 here

        self.V1 = nn.Linear(config.hidden_dim, config.vocab_size)
        init_linear_wt(self.V1)

    def forward(self, x_t, s_t, enc_out, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, sum_temporal_srcs, prev_s, remain_lens, size=config.batch_size):
        ...
        ct_l, len_attn = self.len_attention(dec_h, remain_lens, self.len_attn_table, size)

        ct_d, prev_s = self.dec_attention(dec_h, prev_s)        #intra-decoder attention
        
        p_gen = T.cat([ct_e, ct_d, st_hat, x, ct_l], 1)

        out = T.cat([dec_h, ct_e, ct_d, ct_l], dim=1)   
        ...
        return final_dist, s_t, ct_e, sum_temporal_srcs, prev_s, len_attn


class Model(nn.Module):
    def __init__(self, num=config.batch_size):
