import os
os.environ["CUDA_VISIBLE_DEVICES"] = "1"    #Set cuda device
...
# Set a random seed for random number generator(RNG) in numpy, torch and CUDA to achieve a reproducible result
random.seed(123)
T.manual_seed(123)
...
class Train(object):
    ...
    def save_model(self, iter):
        save_path = config.save_model_path + "/" + self.opt.prefix
        
        if not os.path.isdir(save_path):
            os.mkdir(save_path)

        save_path = save_path + "/%07d.tar" % iter
        ...
        if config.glove:
            ...
            for index in range(config.vocab_size):
                if index < config.num_of_predefined_words:
                    ...
                    # np.append(glove, embedder.get_embedding(self.vocab.id2word(index), append_list=True))
                    np.append(glove, embedder.get_embedding(self.vocab.id2word(index)))

            # Dump embedding to glove dir
            # embedder.dump_embedding(source=False)
    ...
    def train_batch_MLE(self, enc_out, enc_hidden, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, batch):
        ...
        '''
        dec_batch, max_dec_len, dec_lens, target_batch, character_summary_total, character_summary = get_dec_data(batch)                        #Get input and target batchs for training decoder
        ...
        remain_lens = np.copy(character_summary_total)
        ...
        for t in range(min(max_dec_len, config.max_dec_steps)):
            ...
            if t > 0:
                for i in range(len(remain_lens)):
                    remain_lens[i] -= character_summary[i][t-1]
                    
            final_dist, s_t, ct_e, sum_temporal_srcs, prev_s, _ = self.model.decoder(x_t,
                                                                                  s_t,
                                                                                  enc_out,
                                                                                  enc_padding_mask,
                                                                                  ct_e,
    ...
    def train_batch_RL(self, enc_out, enc_hidden, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, article_oovs, batch, greedy):
        ...
        dec_batch, max_dec_len, dec_lens, target_batch, character_summary_total, character_summary = get_dec_data(batch)
        
        remain_lens = np.copy(character_summary_total)

        for t in range(config.max_dec_steps):
            x_t = self.model.embeds(x_t)
            probs, s_t, ct_e, sum_temporal_srcs, prev_s, _ = self.model.decoder(x_t,
                                                                            s_t,
                                                                            enc_out,
                                                                            enc_padding_mask,
                                                                            ct_e,
            ...
            # ----------
            words = x_t.data.tolist()
            for i in range(len(enc_out)):
                oovs = article_oovs[i]

                generated_word = data.outputids2words([words[i]], self.vocab, oovs)

                if remain_lens[i] >= len(generated_word[0]):
                    remain_lens[i] -= len(generated_word[0])
                else:
                    remain_lens[i] = 0
            # ----------
    ...
    def reward_function(self, decoded_sents, original_sents):
        ...
        # We use rouge-l f1 scoure as the reward from Rouge.
        rouge_reward = [score["rouge-l"]["f"] for score in scores]
        

        # Calculate the length difference reward.
        length_reward = []

        for i in range(len(decoded_sents)):    
            model_generate = nltk.word_tokenize(decoded_sents[i])
            gt = nltk.word_tokenize(original_sents[i])

            model_generated_len = len(''.join(model_generate))
            gt_len = len(''.join(gt))

            # Format: (ground truth length, Model_generated_len)
            if abs(gt_len - model_generated_len) <= config.rl_len_reward_diff:
                # Original:
                # len_reward = 1 / (abs(gt_len - model_generated_len) + 1)
                
                # Modified:
                if model_generated_len <= 2 * gt_len:
                    pl = (model_generated_len + config.eps) / (2 * gt_len + config.eps_denominator)
                    len_reward = -(pl * np.log(pl) + (1 - pl) * np.log(1 - pl))
                else:
                    len_reward = 0
            else:
                len_reward = 0

            # divide len_reward by rl_ward_ratio to match ROUGE score's scope.
            len_reward = len_reward / config.rl_reward_ratio
            
            length_reward.append(len_reward)
        
        rouge_reward = get_cuda(T.FloatTensor(rouge_reward))
        length_reward = get_cuda(T.FloatTensor(length_reward))

        # combine rouge reward and length reward as the final reward.

        # Change reward function here:
        # ---------------------------
        # reward = rouge_reward + length_reward
        reward = rouge_reward
    ...
    def train_one_batch(self, batch, iter):
        ...
        article_list, article_lens, enc_padding_mask, enc_batch_extend_vocab, extra_zeros, context, character_articles = get_enc_data(batch)

        # Get word embeddings for all articles
        article_list = self.model.embeds(article_list)                                                    #Get embeddings for encoder input


        # Send articles into encoder.
        enc_out, enc_hidden = self.model.encoder(article_list, character_articles, article_lens)
        ...
        if self.opt.train_rl == "yes":                                                              #perform reinforcement learning training
            ...
            sample_sents, RL_log_probs = self.train_batch_RL(enc_out, enc_hidden, enc_padding_mask, context, extra_zeros, enc_batch_extend_vocab, batch.art_oovs, batch, greedy=False)
            with T.autograd.no_grad():
                # greedy sampling
                greedy_sents, _ = self.train_batch_RL(enc_out, enc_hidden, enc_padding_mask, context, extra_zeros, enc_batch_extend_vocab, batch.art_oovs, batch, greedy=True)
    ...
    def trainIters(self):
        ...
        print('Total Iterations:', config.max_iterations)

        for _ in tqdm(range(config.max_iterations)):
            
            # Get a batch_size of (article, abstract) pairs.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mle_weight', type=float, default=1.0, help='final loss ratio between mle loss and rl loss.')
