...
class Beam(object):
    def __init__(self, start_id, end_id, unk_id, hidden_state, context, target_len, vocab, oovs, idx):
        self.beam_idx = idx                                             # For Debug
        h,c = hidden_state                                              # (n_hid,)

        self.vocab = vocab                                              # We need to get the length of last generated words
        self.oovs = oovs

        # Create beam_size tensors and init them with [START]
        self.tokens = T.LongTensor(config.beam_size,1).fill_(start_id)  #(beam, t) after t time steps

        # Our model here:
        # ---------------
        self.remain_lens = T.LongTensor(config.beam_size, 1).fill_(target_len)  # At first decoding step, remain lens is equal to target lens
        # ---------------
        ...
        self.scores[0][0] = 0                                           #At time step t=0, all beams should extend from a single beam. So, giving high initial score to 1st beam.
    ...
    def advance(self, prob_dist, hidden_state, context, sum_temporal_srcs, prev_s):
        '''
        Perform beam search: Considering the probabilites of given n_beam x n_extended_vocab words,
            select first n_beam words that give high total scores.
        ...
        # Get generated length here:
        best_words_in_words = data.outputids2words([data[0] for data in best_words.data.tolist()], self.vocab, self.oovs)
        best_words_length = [len(word) for word in best_words_in_words]
        ...
        self.remain_lens = self.remain_lens[beams_order]                # sorted

        for i in range(len(self.remain_lens)):
            if config.count_space:
                # print('Count space in beam_search.py')
                if self.remain_lens[i] >= best_words_length[i]:
                    if self.remain_lens[i] >= (best_words_length[i] + 1):
                        self.remain_lens[i] -= (best_words_length[i] + 1)
                    else:
                        self.remain_lens[i] -= best_words_length[i]
                else:
                    self.remain_lens[i] = 0
            else:
                # print('Dont count space in beam_search.py')
                if self.remain_lens[i] >= best_words_length[i]:
                    self.remain_lens[i] -= best_words_length[i]
                else:
                    self.remain_lens[i] = 0
...
def beam_search(enc_hid, enc_out, enc_padding_mask, ct_e, extra_zeros, enc_batch_extend_vocab, model, start_id, end_id, unk_id, target_lens, vocab, oovs):
    """
    Args:
        enc_hid: Encoder Lstm final state + value of cell units for batch size articles.
        enc_out: Encoder hidden states from Bi-LSTM for batch size articles.
    """

    # get h_reduced (Encoder LSTM final hidden states for batch size articles.)
    batch_size = len(enc_hid[0])

    beam_idx = T.LongTensor(list(range(batch_size)))

    #For each example in batch, create Beam object
    beams = [Beam(start_id, end_id, unk_id, (enc_hid[0][i], enc_hid[1][i]), ct_e[i], target_lens[i], vocab, oovs[i], i) for i in range(batch_size)]
    ...
    # For drawing length attention hot plot.
    out_remain_lens = []
    out_len_attns = []
    ...
    for t in range(config.max_dec_steps):
        ...
        # Our model
        # -----------------
        remain_lens_beam = T.stack(
            [beam.remain_lens for beam in beams if beam.done == False]              # Our model
        ).contiguous().view(-1, 1)
        # ------------------
        ...
        # target_lens: list 
        # target_lens : A list contains the length of batch size articles.

        final_dist, (dec_h, dec_c), ct_e, sum_temporal_srcs, prev_s, len_attn = model.decoder(x_t,
                                                                                              s_t,
                                                                                              enc_out_beam,
                                                                                              enc_pad_mask_beam,
                                                                                              ct_e,
                                                                                              ...
                                                                                              prev_s,
                                                                                              remain_lens_beam,
                                                                                              size=len(remain_lens_beam)) 

        if config.len_attn_visualization:
            out_remain_lens.append(remain_lens_beam)
            out_len_attns.append(len_attn)
    ...
    return predicted_words, out_len_attns, out_remain_lens